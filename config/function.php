...
if(!function_exists('upLoadArea')){
   function upLoadArea($avatar_obj,$file_path){
       $file = $avatar_obj;
       if($file->isValid()){
           $client_name = $file->getClientOriginalName();
           $extension  = $file->getClientOriginalExtension();
           if(!$file_path){
               $file_path = \Config::get('system.banner_path');
           }
           if(!file_exists($file_path)) mkdir($file_path,0777,true);
           $new_name    = md5(date('ymdhis').$client_name).".".$extension;
           $path       = $file->move($file_path,$new_name);
...
* 活动图片上传
*/
if(!function_exists('upLoadActivityImg')){
   function upLoadActivityImg($avatar_obj,$file_path,$project_name){
       $file = $avatar_obj;
       if($file->isValid()){
           $client_name = $file->getClientOriginalName();
           $extension  = $file->getClientOriginalExtension();
           if(!$file_path){
               $file_path = \Config::get('system.activity_path');
           }
           if(!file_exists($file_path)) mkdir($file_path,0777,true);
           if(!$project_name){
               $project_name = 'crowdfunding';
           }
           if(!file_exists($project_name)) mkdir($project_name,0777,true);
           $file_path = $file_path.$project_name.'/'.date('Y-m-d').'/';
           $new_name    = md5(date('ymdhis').$client_name).".".$extension;
           $path       = $file->move($file_path,$new_name);
           if($path) return [$new_name,$file_path];
       }
...
if(!function_exists('upLoadGoods')){
   function upLoadGoods($avatar_obj,$is_thump,$width=650,$height=650,$dest_img=''){
       ...
       if($file->isValid()){
            ...
            $new_name    = md5(time().rand(1,999).$client_name).".".$extension;
            $path = $file->move($file_path,$new_name);
           ...
           if($is_thump){
               ...
               $thump_img  = $imgService->thump(base_path()."/public/".$stc_img,$width,$height,base_path()."/public/".$dest_img);
 ...
 */
 ...
 foreach($data as $k => $v)
 {
    if($v['ParentId'] == $pId)
    {         //父亲找到儿子
        $v['ParentId'] = getTree($data, $v['cate_Id']);
        $tree[] = $v;
    }
 }
 return $tree;
...
if (!function_exists('open_wechat_refund')) {
    function open_wechat_refund($total_fee,$refund_fee,$transaction_id){
        \Wechat::setConfig([
            'appId' => \Config::get('wechat.appId'),
            'secret' => \Config::get('wechat.secret'),
            'token' => \Config::get('wechat.token'),
        ...
        return \Wechat::refundConfig($total_fee,$refund_fee,$transaction_id);
    }
}
...
function setPoint($phone_id,$point,$value){
    $phone_info = \App\Models\PhoneModel::find($phone_id);
    if(!$phone_info){
        return -200;
    }
    DB::beginTransaction();
    $phone_info->member_point = $phone_info->member_point+$point;
    $is_save_point = $phone_info->save();
    if(!$is_save_point){
        DB::rollBack();
        return -201;
    }
    
    $PhonePointLogModel = new \App\Models\PhonePointLogModel();
    $PhonePointLogModel->phone_id = $phone_id;
    $PhonePointLogModel->value     = $value;
    $is_save_point_log = $PhonePointLogModel->save();
    if(!$is_save_point_log){
        DB::rollBack();
        return -202;
    }
    DB::commit();
...
if(!function_exists('adminImgUrl')){
    function adminImgUrl($path,$secure = null){
        $img_path = '/new_assets/'.$path;
        return asset($img_path,$secure);
    }
}
if(!function_exists('adminPluginUrl')){
    function adminPluginUrl($path,$secure = null){
        $img_path = '/new_assets/'.$path;
        return asset($img_path,$secure);
    }
}

if(!function_exists('adminJsUrl')){
    function adminJsUrl($path,$secure = null){
        $img_path = '/new_assets/'.$path;
        return asset($img_path,$secure);
    }
}

if(!function_exists('adminCssUrl')){
    function adminCssUrl($path,$secure = null){
        $img_path = '/new_assets/'.$path;
        return asset($img_path,$secure);
    }
}
...
//secure_asset
if(!function_exists('adminLoginImgUrl')){
    function adminLoginImgUrl($path,$secure = null){
        $img_path = '/back/images/'.$path;
        return asset($img_path,$secure);
    }
}
if(!function_exists('adminFontsUrl')){
    function adminFontsUrl($path,$secure = null){
        $fonts_path = '/fonts/'.$path;
        return asset($fonts_path,$secure);
    }
}

if(!function_exists('adminLoginJsUrl')){
    function adminLoginJsUrl($path,$secure = null){
        $img_path = '/back/js/'.$path;
        return asset($img_path,$secure);
    }
}

if(!function_exists('adminLoginCssUrl')){
    function adminLoginCssUrl($path,$secure = null){
        $img_path = '/back/css/'.$path;
        return asset($img_path,$secure);
    }
}

if(!function_exists('adminLog')){
    function adminLog($user_info,$log,$type = null,$input = null){ 
        $save['user_id']  = $user_info['id'];
        $save['email']    = $user_info['email'];
        $save['nick_name']= $user_info['nick_name'];
        $save['group_id'] = $user_info['group_id'];
        $save['role_id']  = $user_info['role_id'];
        $save['create_time'] = time();
        $save['log']      = $log;
        $save['type']      = $type;
        $save['input']      = $input;
        $AdminUsersLogsModel = new \App\Models\Admin\AdminUsersLogsModel();
        return $AdminUsersLogsModel->setLog($save);
    }
}


if(!function_exists('supplierLog')){
    function supplierLog($user_info,$log,$type = null,$input = null){
        $save['supplier_id']  = $user_info['id'];
        $save['email']    = $user_info['supplier_account'];
      //  $save['nick_name']= $user_info['nick_name'];
      //  $save['group_id'] = $user_info['group_id'];
      //  $save['role_id']  = $user_info['role_id'];
        $save['create_time'] = time();
        ...
        $save['type']      = $type;
        $save['input']      = $input;
        $AdminUsersLogsModel = new \App\Models\Supplier\SupplierUsersLogsModel();
        return $AdminUsersLogsModel->setLog($save);
    }
}
